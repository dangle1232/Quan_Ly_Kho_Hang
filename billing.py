...
class BillClass:
    def __init__(self, root):
        ...
        # clock===============
        self.lbl_clock = Label(self.root, text="Welcome to Kho QTC \t\t Date:DD-MM-YYYY\t\t Time :HH:MM:SS",
        ...
        btn_search= Button(ProductFrame2, text="Search",command=self.search,font=("goudy old style",15),bg="red",fg="white",cursor="hand2").place(x=280,y=47,width=100,height=22)
        
        btn_show_all= Button(ProductFrame2, text="Show All ",command=self.show,font=("goudy old style",15),bg="black",fg="white",cursor="hand2").place(x=285,y=10,width=100,height=22)
        ...
        self.productTable = ttk.Treeview( ProductFrame3, column=("pid", "name", "price", "qty","status","des"),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
        ...
        self.productTable.column("des",width=150)
        self.productTable.pack(fill=BOTH,expand=1)
        self.productTable.bind("<ButtonRelease-1>",self.get_data)
        lbl_note=Label(ProductFrame3,text="Note :'Enter 0 quantity to remove product from the cart'",font=("goudy old style",10),bg="white",fg="red").pack(side=BOTTOM,fill=X )
        ...
        btn_7= Button (Cal_Frame,text=7,command=lambda:self.get_input(7),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=1,column=0)
        btn_8= Button (Cal_Frame,text=8,command=lambda:self.get_input(8),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=1,column=1)
        btn_9= Button (Cal_Frame,text=9,command=lambda:self.get_input(9),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=1,column=2)
        btn_sum= Button (Cal_Frame,text='+',command=lambda:self.get_input('+'),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=1,column=3)
        ...
        btn_3= Button (Cal_Frame,text=3,command=lambda:self.get_input(3),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=3,column=0)
        btn_2= Button (Cal_Frame,text=2,command=lambda:self.get_input(2),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=3,column=1)
        btn_1= Button (Cal_Frame,text=1,command=lambda:self.get_input(1),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=3,column=2)
        btn_mul= Button (Cal_Frame,text='*',command=lambda:self.get_input('*'),font=("arial",15,"bold"),bd=5,width=4,pady=10,cursor="hand2").grid(row=3,column=3)
        
        btn_0= Button (Cal_Frame,text=0,command=lambda:self.get_input(0),font=("arial",15,"bold"),bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=0)
        btn_c= Button (Cal_Frame,text='c',command=self.clean_cal,font=("arial",15,"bold"),bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=1)
        btn_equal= Button (Cal_Frame,text='=',command=self.perform_cal,font=("arial",15,"bold"),bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=2)
        btn_div= Button (Cal_Frame,text='/',command=lambda:self.get_input('/'),font=("arial",15,"bold"),bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=3)
        ...
        txt_price_qty =Entry(Add_CartWidgetsFrame,textvariable=self.var_price, font=("times new roman",15,),bg="lightyellow",state='readonly').place(x=230,y=35,width=150,height=22)
        
        lbl_p_qty =Label(Add_CartWidgetsFrame,text="Quantity", font=("times new roman",15,"bold"),bg="white").place(x=390,y=5)
        ...
        self.lbl_instock=Label(Add_CartWidgetsFrame,text="In Stock [100]", font=("times new roman",15,"bold"),bg="white")
        ...
        footer=Label(self.root,text="QTC-Inventory Management System",font=("times new roman",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
        self.show()
        #self.bill_top()
    ...
    def show (self):
            ...
            try:
                # self.productTable = ttk.Treeview( ProductFrame3, column=("pid", "name", "price", "qty","status","des"),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
                cur.execute("Select pid, name,price, qty,status, des from product where status ='Active' ")
                rows= cur.fetchall()
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "":
                ...
                cur.execute("SELECT pid, name,price, qty,status, des from product  where name LIKE'%" + self.var_search.get() + "%' and status ='Active'")
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.productTable.delete(*self.productTable.get_children())
                    for row in rows:
    ...
    def generate_bill(self):
     if self.var_cname.get() == '' or self.var_contact.get() == '':
        ...
        fp.close()
        messagebox.showinfo("Saved","Bill has been generated/Save in Data",parent=self.root)
        self.chk_print=1
    ...
    def bill_middle(self):
        ...
        try:         
            for row in self.cart_list:
                ...
                if qty == 1:
                 messagebox.showwarning("Warning", f"Vui long nhap them hang {self.var_pname.get()} ", parent=self.root)
